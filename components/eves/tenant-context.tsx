...
export const reportOptions = [
  { key: "projectTags", label: "Project Tags", group: "Regulatory Reports", href: "/reports/project-tags" },
  { key: "generation", label: "Generation", group: "Regulatory Reports", href: "/reports/regulatory" },
  { key: "sessions", label: "Charging Sessions", group: "Master Reports", href: "/reports/charging-sessions" },
  { key: "intervals", label: "Interval Load Profile", group: "Master Reports", href: "/reports/interval-load-profile" },
  { key: "throughput", label: "Infrastructure & Throughput", group: "Master Reports", href: "/reports/throughput" },
  { key: "uptime", label: "Uptime & Reliability", group: "Master Reports", href: "/" },
] as const;
export type ReportKey = (typeof reportOptions)[number]["key"];
export function tenantReports(tenant: Tenant) { return reportOptions.filter(r => tenant.components["Reports/Analytics"] && tenant.reports[r.key]); }
export type Tenant = {
...
export const blankTenant = (): Tenant => ({ reports: Object.fromEntries(reportOptions.map(r => [r.key, true])) as Record<ReportKey, boolean>, id: "", name: "", subdomain: "", email: "", analytics: [...analyticsOptions], regulatory: true, master: true, address1: "", address2: "", postalCode: "", city: "", department: "", region: "", country: "", contact: "", latitude: "", longitude: "", createdOn: "", createdBy: "", changedOn: "", changedBy: "", components: Object.fromEntries(componentNames.map(n => [n, true])) });
const referenceRows = [
...
function readSaved(): Tenant[] {
  try {
    ...
    if (Array.isArray(value) && value.every(t => t && typeof t.id === "string" && typeof t.name === "string" && typeof t.email === "string" && typeof t.subdomain === "string" && typeof t.regulatory === "boolean" && typeof t.master === "boolean" && Array.isArray(t.analytics))) return value.map(t => ({ ...blankTenant(), ...t, reports: Object.fromEntries(reportOptions.map(r => [r.key, typeof t.reports?.[r.key] === "boolean" ? t.reports[r.key] : r.group === "Regulatory Reports" ? t.regulatory : t.master])) as Record<ReportKey, boolean>, components: { ...blankTenant().components, ...t.components } }));
    const old = JSON.parse(localStorage.getItem("eves-tenant-configuration-v1") ?? "null");
    if (Array.isArray(old)) return initial.map((t, i) => ({ ...t, ...(old[i] ? { reports: Object.fromEntries(reportOptions.map(r => [r.key, r.group === "Regulatory Reports" ? old[i].regulatory === true : old[i].master === true])) as Record<ReportKey, boolean>, regulatory: old[i].regulatory === true, master: old[i].master === true, analytics: Array.isArray(old[i].analytics) ? old[i].analytics : t.analytics } : {}) }));
...
export function TenantProvider({ children }: { children: React.ReactNode }) {
  ...
  return <TenantContext.Provider value={{ tenantView, setTenantView, tenants, active: tenants.find(t => t.id === activeId) ?? tenants[0] ?? { ...blankTenant(), name: "No tenant selected", reports: Object.fromEntries(reportOptions.map(r => [r.key, false])) as Record<ReportKey, boolean>, regulatory: false, master: false }, selectTenant, saveTenant, refreshTenants: () => setRecords(readSaved()), deleteTenant: id => persist(records.map(t => t.id === id ? { ...t, deleted: true } : t)), restoreTenant: id => { persist(records.map(t => t.id === id ? { ...t, deleted: false } : t)); } }}>{children}</TenantContext.Provider>;
}
